...
for x = range(-1.25,1.25,np)
    ...
    P, indIntersect, T, det_vals = ray_triangle_intersect(F,V,ray_origin,ray_vector; rayType = :ray, triSide = 1)
    println(det_vals)
...
for x = range(-1.25,1.25,np)
    ray_origin = Point3{Float64}(x,0.25*sin(x*pi),1.25)    
    P, indIntersect, T, det_vals = ray_triangle_intersect(F,V,ray_origin,ray_vector; rayType = :ray, triSide = 0)
    scatter!(ax1,ray_origin,markersize = markerSize,color=:blue)    
    scatter!(ax1,P,markersize = markerSize,color=:red)
    lines!(ax1,[ray_origin,ray_origin.+ray_vector],color=:blue)
    meshplot!(ax1, F[indIntersect], V, color=:green, strokecolor=:green, strokewidth=2)
...
for x = range(-1.25,1.25,np)
    ...
    P, indIntersect, T, det_vals = ray_triangle_intersect(F,V,ray_origin,ray_vector; rayType = :line, triSide = 1)
    scatter!(ax1,ray_origin,markersize = markerSize,color=:blue)
...
for x = range(-1.25,1.25,np)
    ...
    P, indIntersect, T, det_vals = ray_triangle_intersect(F,V,ray_origin,ray_vector; rayType = :line, triSide = 0)
    scatter!(ax1,ray_origin,markersize = markerSize,color=:blue)
    scatter!(ax1,ray_origin.+ray_vector,markersize = markerSize,color=:blue)
    scatter!(ax1,P,markersize = markerSize,color=:red)
    lines!(ax1,[ray_origin,ray_origin.+ray_vector],color=:blue)
